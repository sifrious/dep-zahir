...
Both operations are retry-safe and return `replayed`. A revoked connection remains attached to its original opaque account, so resolution returns the frozen `provider_failed` outcome with reason `provider_revoked` rather than creating another account. After accepted recovery, require a fresh authentication before a product creates a local session.
