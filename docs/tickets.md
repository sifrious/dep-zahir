...
| ACC-017 | Integrate the Burd design system | Public account and trust surfaces consume the standalone Burd package | Published v0.1.0 package | Complete |
...
| ACC-023 | Publish product behavior references | Every documented product has a public, status-labeled reference for purpose, inputs, workflow, outputs, interactions, data boundaries, entitlements, and limits | Accurate product contract | Complete for all current packages |
| ACC-024 | Publish Free and Paid tiers for every package product | Every product page, pricing page, and billing page presents separate capability lists; paid tiers remain gated on real prices and Stripe Price IDs | Accurate commercial decisions | Structure complete; paid prices required |
